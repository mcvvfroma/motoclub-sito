...
import { Edit, Trash2, ShieldAlert, Loader2, Plus, User, Mail } from "lucide-react"
...
export default function MembersPage() {
    ...
    try {
      ...
      if (user.status !== 'admin') {
        toast({
          ...
          description: "Solo gli amministratori possono gestire l'anagrafica.",
        })
        router.push("/")
        return
      }
  ...
  const handleAddMember = () => {
    ...
    toast({ title: "Socio aggiunto", description: `${newMember.nome} ${newMember.cognome} è stato registrato.` })
  }
  ...
  const handleEditMember = () => {
    ...
    setFormData({ nome: "", cognome: "", email: "", status: "socio" })
    toast({ title: "Socio aggiornato", description: "Le modifiche sono state salvate." })
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background text-foreground">
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground font-medium">Verifica permessi in corso...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen pb-20 md:pb-0 md:pt-16 bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <Badge className="mb-4 bg-primary/10 text-primary border-none font-bold uppercase tracking-wider">Area Amministrativa</Badge>
            <h1 className="text-4xl font-headline font-bold mb-2 text-foreground">Anagrafica Soci</h1>
            <p className="text-muted-foreground max-w-2xl">Gestione completa dei membri del Motoclub VVF Roma.</p>
          </div>
          ...
          <Dialog open={isAdding} onOpenChange={setIsAdding}>
            <DialogTrigger asChild>
              <Button className="bg-destructive hover:bg-destructive/90 gap-2 h-12 px-6 rounded-full shadow-lg shadow-destructive/20 font-bold uppercase tracking-tighter">
                 <Plus className="w-5 h-5" /> + AGGIUNGI NUOVO SOCIO
              </Button>
            </DialogTrigger>
            <DialogContent className="bg-card border-border sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle className="text-2xl font-headline">Nuovo Socio</DialogTitle>
                <DialogDescription>Inserisci i dati del collega per la registrazione.</DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter>
                ...
                <Button onClick={handleAddMember} className="bg-primary text-white font-bold">Salva</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </header>
        ...
        <Card className="bg-card border-border overflow-hidden shadow-2xl">
          <CardHeader className="bg-secondary/30 border-b border-border">
            <CardTitle className="text-xl">Elenco Colleghi</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <Table>
              <TableHeader className="bg-secondary/50">
                <TableRow className="border-border">
                  <TableHead className="text-accent font-bold uppercase text-xs tracking-widest">Nome</TableHead>
                  <TableHead className="text-accent font-bold uppercase text-xs tracking-widest">Cognome</TableHead>
                  <TableHead className="text-accent font-bold uppercase text-xs tracking-widest">Email</TableHead>
                  <TableHead className="text-accent font-bold uppercase text-xs tracking-widest">Status</TableHead>
                  <TableHead className="text-accent font-bold uppercase text-xs tracking-widest text-right">Azioni</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {members.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-12 text-muted-foreground italic">
                      Caricamento soci o nessun dato trovato...
                    </TableCell>
                  </TableRow>
                ) : (
                  members.map((socio: any) => (
                    <TableRow key={socio.email} className="hover:bg-primary/5 transition-colors border-border">
                      <TableCell className="font-medium text-foreground">{socio.nome}</TableCell>
                      <TableCell className="font-medium text-foreground">{socio.cognome}</TableCell>
                      <TableCell className="text-muted-foreground font-mono text-xs">{socio.email}</TableCell>
                      <TableCell>
                        <Badge 
                            ...
                            <AlertDialogContent className="bg-card border-border">
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Vuoi davvero eliminare questo socio? Questa operazione non può essere annullata.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
          ...
          <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMember(null)}>
            <DialogContent className="bg-card border-border sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>Aggiorna i dati di {editingMember.nome}.</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                ...
                <div className="grid gap-2">
                  ...
                  <Input id="edit-email" type="email" value={formData.email} className="bg-background border-border opacity-50" disabled />
                </div>
                <div className="grid gap-2">
